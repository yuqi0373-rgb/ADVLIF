...
def compute_swd(orig_images, mask_images, device):
    orig_images = torch.as_tensor(orig_images.transpose([0, 3, 1, 2]).astype(np.float32) / 255.0)
    mask_images = torch.as_tensor(mask_images.transpose([0, 3, 1, 2]).astype(np.float32) / 255.0)

    out = swd(orig_images, mask_images, device=device)  # Fast estimation if device="cuda"
    return out
