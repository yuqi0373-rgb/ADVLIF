...
class BaseModel(ABC):
    ...
    def get_current_visuals(self):
        """Return visualization images. Errors are ignored, only valid images are returned."""
        ...
        for name in self.visual_names:
            ...
            try:
                # If the attribute exists directly
                if hasattr(self, name):
                    visual_ret[name] = getattr(self, name)
                    continue

                # Try resolving composed image names
                if len(name.split('_')) != 2:
                    if self.opt.model == 'ParsiLIF':
                        img_name = name[:-1] + '_' + name[-1]
                        visual_ret[name] = getattr(self, img_name)
                    elif self.opt.model == 'CycleGAN':
                        base_name = name.split('_')[0] + '_' + name.split('_')[1]
                        l_output = getattr(self, base_name, [])
                        if len(l_output) > 0:
                            visual_ret[name] = l_output[int(name.split('_')[-1]) - 1]
                    else:
                        base_name = name.split('_')[0] + '_' + name.split('_')[1]
                        visual_ret[name] = getattr(self, base_name)[int(name.split('_')[-1]) - 1]
                else:
                    # Handles names like "real_1", "fake_2"
                    list_name = name.split('_')[0]
                    index = int(name.split('_')[-1]) - 1
                    visual_ret[name] = getattr(self, list_name)[index]
    ...
    def get_current_losses(self):
        ...
        for name in self.loss_names:
            ...
            try:
                ...
                # 情况2：loss_GAN_1 → 对应 self.loss_GAN[0]
                elif len(name.split('_')) == 2:
                    base, idx = name.split('_')
                    attr = getattr(self, 'loss_' + base, None)
                    errors_ret[name] = float(attr[int(idx) - 1]) if attr is not None else 0.0

                # 情况3：loss_G_GAN_2 → 对应 self.loss_G_GAN[1]
                elif len(name.split('_')) == 3:
                    part1, part2, idx = name.split('_')
                    attr_name = f'loss_{part1}_{part2}'
