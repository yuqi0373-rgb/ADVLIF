...
from Model.util.util import tensor_to_pil
from Model.data import transform
from Model.util.postprocessing import compute_final_results, compute_cell_results
from Model.util.postprocessing import encode_cell_data_v4, decode_cell_data_v4
from Model.options import Options, print_options

from .base_model import BaseModel
from .ParsiLIF_model import ParsiLIFModel
...
def run_torchserve(img, model_path=None, eager_mode=False, opt=None, seg_only=False):
    ...
    res = requests.post(
        f'{torchserve_host}/wfpredict/Model',
...
def run_dask(img, model_path, eager_mode=False, opt=None, seg_only=False):
    model_dir = os.getenv('ParsiLIF_MODEL_DIR', model_path)
...
def infer_modalities(img, tile_size, model_dir, eager_mode=False,
    ...
    if not hasattr(opt,'seg_gen') or (hasattr(opt,'seg_gen') and opt.seg_gen): # the first condition accounts for old settings of Model; the second refers to deepliifext models
