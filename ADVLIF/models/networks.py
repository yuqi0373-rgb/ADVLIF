...
class ResnetBlock(nn.Module):
    """A clearer, more extensible 2-conv ResNet block with skip-connection."""
    def __init__(self,
                 dim: int,
        ...
        # --- helper to wrap conv with spectral norm if requested ---
        def make_conv(in_ch, out_ch):
            conv = nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=0, bias=use_bias)
            return spectral_norm(conv) if use_spectral_norm else conv

        # --- choose padding layer once (reuse) ---
        if padding_type == 'reflect':
            Pad = nn.ReflectionPad2d
        elif padding_type == 'replicate':
            Pad = nn.ReplicationPad2d
        elif padding_type == 'zero':
            Pad = lambda p: nn.ZeroPad2d(p)
        else:
            raise ValueError(f"Unrecognized padding_type: {padding_type}")

        # Layer 1
        self.pad1   = Pad(1)
        self.conv1  = make_conv(dim, dim)
...
def _get_padding(pad_type: str, padding: int):
    return nn.ReflectionPad2d(padding) if pad_type == 'reflect' else nn.ZeroPad2d(padding)
