...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        # ---- NEW: save segmentation masks as 1-channel ----
        if 'B_5' in label or 'seg' in label.lower():
            # ensure 0–255 uint8 single channel
            #if im.ndim == 3 and im.shape[2] > 1:
            #    im = cv2.cvtColor(im, cv2.COLOR_RGB2GRAY)
            #im = (im if im.dtype == np.uint8 else np.clip(im, 0, 255).astype(np.uint8))
            im = ((im >150)*255).astype(np.uint8)
            imsave(save_path, im)
        else:
...
def save_images0(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    """Save images to the disk.
...
class Visualizer():
    ...
    def plot_current_losses(self, epoch, counter_ratio, losses):
        ...
        if self.remote:
            ...
            if self.printer: print('update training loss')
            ...
            if self.printer: print('update validation loss')
            ...
            if self.printer: print('update other metrics')
            ...
            try:
                if self.plot_data_update_train:
                    if self.printer: print('plotting train loss')
                    self.vis.line(
    ...
    def print_current_losses(self, epoch, iters, losses):
        ...
        if self.printer: print(message)  # print the message
        with open(self.log_name, "a") as log_file:
